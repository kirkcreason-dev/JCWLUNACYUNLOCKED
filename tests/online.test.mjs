...
test('online room joins load both chosen wrestlers and host arena before starting once',async t=>{
  const {db,a,b,code}=await pair(t),meta=db.get(`rooms/LU210-${code}/meta`);
...
test('Caleb, Kongo, and Father Bronson join and host with the full roster; incompatible rosters are rejected',async()=>{
 for(const character of ['caleb-konley','kongo-kong','father-bronson','hokane','steven-flowe','ec3','krule','jeeves','atiba','jp-grayson','tommy','shaggy-2-dope','jacksyn','dj-clay','jeff-lane','shane-mercer','josh-bishop','ring-rat','green-phantom','evil-dead']){
 const caleb=roster.findIndex(f=>f.id===character);assert.equal(caleb,character==='caleb-konley'?15:character==='kongo-kong'?21:character==='father-bronson'?22:character==='hokane'?23:character==='steven-flowe'?24:character==='ec3'?25:character==='krule'?26:character==='jeeves'?27:28+['atiba','jp-grayson','tommy','shaggy-2-dope','jacksyn','dj-clay','jeff-lane','shane-mercer','josh-bishop','ring-rat','green-phantom','evil-dead'].indexOf(character));
 for(const hostCaleb of [true,false]){
  ...
  try{
   ...
   assert.ok(buffer.receive(packet,0));buffer.apply(guest,50);assert.equal(guest.fighters[hostCaleb?0:1].definition.id,character);
...
test('two simultaneous joiners cannot claim the same guest seat',async t=>{
  ...
  assert.equal(results.filter(r=>r.status==='fulfilled').length,1);assert.ok(db.get(`rooms/LU210-${code}`));assert.equal(a.log.started,1);
});
test('invalid room input never changes another room',async t=>{
  ...
  assert.equal(db.get(`rooms/LU210-${code}/meta/host/id`),a.session.ctx.id);
  assert.equal(roomCode(' lu210-abcd '),'ABCD');
});
test('quick match atomically pairs searchers without touching the legacy queue',async t=>{
  ...
  assert.deepEqual(db.get('quickQueue'),{code:'OLDX',t:1});assert.equal(db.get('rooms/LU210-queue'),null);
});
test('cancelling quick match removes only its own room and queue claim',async()=>{
  ...
  await db.ref('rooms/LU210-queue').set({code:'ZZZZ',owner:'someone-else',at:Date.now()});await a.session.leave();
  assert.equal(db.get(`rooms/LU210-${code}`),null);assert.equal(db.get('rooms/LU210-queue/owner'),'someone-else');assert.equal(db.listeners.size,0);
});
...
test('host disconnect cleanup removes the room and notifies the guest',async t=>{
  ...
  const cleanup=[...db.disconnects].find(item=>item.ref.path===`rooms/LU210-${code}`);
  assert.ok(cleanup);assert.equal(cleanup.value,null);
  db.write(cleanup.ref.path,cleanup.value);await settle();
  assert.equal(db.get(`rooms/LU210-${code}`),null);assert.equal(b.log.ended.length,1);
});
