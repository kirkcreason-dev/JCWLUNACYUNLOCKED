...
test('online room joins load both chosen wrestlers and host arena before starting once',async t=>{
  const {db,a,b,code}=await pair(t),meta=db.get(`rooms/LU140-${code}/meta`);
...
test('Caleb, Kongo, and Father Bronson join and host with the full roster; incompatible rosters are rejected',async()=>{
 ...
 for(const hostCaleb of [true,false]){
  ...
  try{
   ...
   const meta=db.get(`rooms/LU140-${code}/meta`);
...
test('two simultaneous joiners cannot claim the same guest seat',async t=>{
  ...
  assert.equal(results.filter(r=>r.status==='fulfilled').length,1);assert.ok(db.get(`rooms/LU140-${code}`));assert.equal(a.log.started,1);
});
test('invalid room input never changes another room',async t=>{
  ...
  assert.equal(db.get(`rooms/LU140-${code}/meta/host/id`),a.session.ctx.id);
  assert.equal(roomCode(' lu140-abcd '),'ABCD');
});
test('quick match atomically pairs searchers without touching the legacy queue',async t=>{
  ...
  assert.deepEqual(db.get('quickQueue'),{code:'OLDX',t:1});assert.equal(db.get('rooms/LU140-queue'),null);
});
test('cancelling quick match removes only its own room and queue claim',async()=>{
  ...
  await db.ref('rooms/LU140-queue').set({code:'ZZZZ',owner:'someone-else',at:Date.now()});await a.session.leave();
  assert.equal(db.get(`rooms/LU140-${code}`),null);assert.equal(db.get('rooms/LU140-queue/owner'),'someone-else');assert.equal(db.listeners.size,0);
});
...
test('host disconnect cleanup removes the room and notifies the guest',async t=>{
  ...
  const cleanup=[...db.disconnects].find(item=>item.ref.path===`rooms/LU140-${code}`);
  assert.ok(cleanup);assert.equal(cleanup.value,null);
  db.write(cleanup.ref.path,cleanup.value);await settle();
  assert.equal(db.get(`rooms/LU140-${code}`),null);assert.equal(b.log.ended.length,1);
});
