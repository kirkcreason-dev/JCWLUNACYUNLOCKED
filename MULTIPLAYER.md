...
New rooms use `rooms/LU120-ABCD`, with a `lunacy-2d-v12` protocol marker. Quick Match uses `rooms/LU120-queue`. Both players must use v0.12.0 with all 36 fighters. Incompatible older clients are isolated by the new namespace. The old `quickQueue` and old unprefixed rooms are not modified.
