...
for(const id of [39])atlases[id]=await loadImage(await readFile(new URL(`${roster[id].id}.png`,assets)));
...
for(const id of [39]){
 ...
 for(let n=0;n<cases.length;n++){
  const label=cases[n],m=new Match(roster,id,39,{mode:'local'});m.phase='fight';const [a,b]=m.fighters;a.x=515;b.x=620;
...
for(const config of [{width:390,height:844,safe:{top:47,bottom:34}},{width:844,height:390,safe:{left:47,right:47,bottom:21}}]){
 ...
 const m=new Match(roster,39,39,{mode:'local'});m.phase='fight';m.fighters[0].x=510;m.fighters[1].x=625;step(m,1,{heavy:true});step(m,15);r.draw(m,0,0);
 await writeFile(new URL(`evil-dead-${portrait?'portrait':'landscape'}.png`,out),c.toBuffer('image/png'));
}
console.log('PASS: rendered Evil Dead across 12 combat states and two phone canvas layouts.');
