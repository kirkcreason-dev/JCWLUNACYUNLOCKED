...
for(const id of [22,23])atlases[id]=await loadImage(await readFile(new URL(`${roster[id].id}.png`,assets)));
...
for(let i=0;i<cases.length;i++){
 ...
 const canvas=createCanvas(size.width,size.height),r=new Renderer(canvas,roster,atlases,arenas);r.portrait=portrait;r.compact=!portrait;r.lowPower=true;r.reduced=true;r.scheme='touch';
 const m=new Match(roster,23,22,{mode:'local'});m.phase='fight';m.fighters[0].x=i===3?250:490;m.fighters[1].x=i===3?1060:650;m.fighters[1].hp=64;m.fighters[0].meter=100;
 r.draw(m,0,0);
...
await writeFile(new URL('phone-review.png',out),contact.toBuffer('image/png'));
