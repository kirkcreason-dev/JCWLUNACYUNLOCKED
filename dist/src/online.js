...
export function roomCode(value){return String(value||'').trim().toUpperCase().replace(/^LU6-/, '');}
export function validRoom(meta,now=Date.now()){
  return meta?.protocol===PROTOCOL&&Number.isInteger(meta.host?.fighter)&&meta.host.fighter>=0&&meta.host.fighter<21
    ...
    &&now-meta.created<6*60*60*1000&&(!meta.guest||(typeof meta.guest.id==='string'&&Number.isInteger(meta.guest.fighter)&&meta.guest.fighter>=0&&meta.guest.fighter<21));
}
...
export class OnlineSession{
  ...
  async create(ctx,fighter,arena){
    ...
    for(let attempt=0;attempt<8;attempt++){
      this.check(ctx);ctx.code=this.makeCode();ctx.ref=ctx.db.ref(`rooms/LU6-${ctx.code}`);
  ...
  async join(ctx,value,fighter){
    ...
    ctx.role='guest';ctx.code=code;ctx.ref=ctx.db.ref(`rooms/LU6-${code}`);
