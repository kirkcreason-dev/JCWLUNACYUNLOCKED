import {PROTOCOL,InputPackets,RemoteInput,packSnapshot} from './online-protocol.js?v=0.14.0';

const QUEUE='rooms/LU140-queue';
const noop=()=>{};
...
export function roomCode(value){return String(value||'').trim().toUpperCase().replace(/^LU140-/, '');}
export function validRoom(meta,now=Date.now()){
  return meta?.protocol===PROTOCOL&&Number.isInteger(meta.host?.fighter)&&meta.host.fighter>=0&&meta.host.fighter<36
    ...
    &&now-meta.created<6*60*60*1000&&(!meta.guest||(typeof meta.guest.id==='string'&&Number.isInteger(meta.guest.fighter)&&meta.guest.fighter>=0&&meta.guest.fighter<36));
}
...
export class OnlineSession{
  ...
  async create(ctx,fighter,arena){
    ...
    for(let attempt=0;attempt<8;attempt++){
      this.check(ctx);ctx.code=this.makeCode();ctx.ref=ctx.db.ref(`rooms/LU140-${ctx.code}`);
  ...
  async join(ctx,value,fighter){
    ...
    ctx.role='guest';ctx.code=code;ctx.ref=ctx.db.ref(`rooms/LU140-${code}`);
