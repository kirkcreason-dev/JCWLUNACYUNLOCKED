...
export function roomCode(value){return String(value||'').trim().toUpperCase().replace(/^LU120-/, '');}
...
export class OnlineSession{
  ...
  async create(ctx,fighter,arena){
    ...
    for(let attempt=0;attempt<8;attempt++){
      this.check(ctx);ctx.code=this.makeCode();ctx.ref=ctx.db.ref(`rooms/LU120-${ctx.code}`);
  ...
  async join(ctx,value,fighter){
    ...
    ctx.role='guest';ctx.code=code;ctx.ref=ctx.db.ref(`rooms/LU120-${code}`);
