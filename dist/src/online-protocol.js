...
const FIELDS=['x','z','vz','vx','facing','hp','meter','guard','t','invincible','downTime','combo','walkDirection','fallDuration','divePower','chain','reversalWindow','reversalCooldown','reboundTime','reboundDirection','ropeTime','weaponUses'];
...
export function packSnapshot(match,seq,events=[]){
  return {protocol:PROTOCOL,seq,round:match.round,wins:[...match.wins],phase:match.phase,phaseTime:match.phaseTime,
    ...
    fighters:match.fighters.map(f=>({id:f.id,state:f.state,weapon:f.weapon||'chair',weaponWear:Object.fromEntries(['chair','bat','guitar','trashcan','bottle'].map(k=>[k,f.weaponWear?.[k]||0])),attackStyle:f.attackStyle||'chair',move:f.move||null,fallFace:f.fallFace||'back',exhausted:Boolean(f.exhausted),confirmed:Boolean(f.confirmed),secondWindUsed:Boolean(f.secondWindUsed),...Object.fromEntries(FIELDS.map(k=>[k,Number.isFinite(f[k])?f[k]:0]))})),
    pole:match.pole?clone(match.pole):null,grapple:match.grapple?clone(match.grapple):null,pin:match.pin?clone(match.pin):null,events:clone(events)};
}
export function validSnapshot(s,ids){
  ...
  if(!s.fighters.every((f,i)=>f.id===ids[i]&&STATES.has(f.state)&&['none','chair','bat','guitar','trashcan','bottle'].includes(f.weapon)&&['light','chair','bat','guitar','trashcan','bottle','kick'].includes(f.attackStyle)&&[null,undefined,'light','heavy','special','bat','guitar','trashcan','bottle','counter','aerial'].includes(f.move)&&['front','back'].includes(f.fallFace)&&typeof f.exhausted==='boolean'&&typeof f.confirmed==='boolean'&&Number.isInteger(f.chain)&&finite(f.chain,0,2)&&FIELDS.every(k=>finite(f[k],-100000,100000))&&finite(f.x,200,1080)&&finite(f.z,0,1000)&&finite(f.hp,0,100)&&finite(f.meter,0,100)&&finite(f.guard,-100,100)&&[-1,1].includes(f.facing)))return false;
  if(!s.fighters.every(f=>f.weaponWear&&typeof f.weaponWear==='object'&&!Array.isArray(f.weaponWear)&&Object.entries(f.weaponWear).every(([k,v])=>['chair','bat','guitar','trashcan','bottle'].includes(k)&&Number.isInteger(v)&&finite(v,0,6))))return false;
  if(s.pole&&(!finite(s.pole.x,200,1080)||typeof s.pole.claimed!=='boolean'||typeof s.pole.broken!=='boolean'||![null,0,1].includes(s.pole.holder??null)||![null,0,1].includes(s.pole.claimant??null)||!finite(s.pole.progress,0,1.05)||!finite(s.pole.uses,0,4)||(s.pole.looseX!=null&&!finite(s.pole.looseX,200,1080))))return false;
  if(s.grapple?.kind==='superplex'&&(!finite(s.grapple.corner,200,1080)||![-1,1].includes(s.grapple.throwDir)||!Array.isArray(s.grapple.origins)||s.grapple.origins.length!==2||!s.grapple.origins.every(o=>finite(o.x,200,1080)&&finite(o.z,0,140))))return false;
...
const EVENT_TYPES=new Set(['poleClaim','poleDrop','poleHint','attackActive','round','fight','hit','slam','block','special','notReady','throwBreak','holdBreak','kickout','guardBreak','count','roundEnd','matchEnd','jump','land','whiff','throw','pin','weapon','weaponBreak','taunt','tauntStart','climb','dive','ropeBreak','pinRelease','grapple','swing','reversal','secondWind','ropeRebound']);
export class SnapshotBuffer{
  ...
  apply(match,now){
    ...
    match.pole=s.pole?{holder:null,claimant:null,looseX:null,...clone(s.pole)}:null;match.winner=s.winner??null;match.roundWinner=s.roundWinner??null;match.grapple=s.grapple?clone(s.grapple):null;match.pin=s.pin?clone(s.pin):null;
