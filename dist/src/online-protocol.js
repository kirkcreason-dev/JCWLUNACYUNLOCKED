...
export const PROTOCOL='lunacy-2d-v4';
...
const FIELDS=['x','z','vz','vx','facing','hp','meter','guard','t','invincible','downTime','combo','walkDirection','fallDuration','divePower'];
...
export function packSnapshot(match,seq,events=[]){
  return {protocol:PROTOCOL,seq,round:match.round,wins:[...match.wins],phase:match.phase,phaseTime:match.phaseTime,
    ...
    fighters:match.fighters.map(f=>({id:f.id,state:f.state,weapon:f.weapon||'chair',attackStyle:f.attackStyle||'chair',move:f.move||null,fallFace:f.fallFace||'back',exhausted:Boolean(f.exhausted),...Object.fromEntries(FIELDS.map(k=>[k,Number.isFinite(f[k])?f[k]:0]))})),
...
const EVENT_TYPES=new Set(['round','fight','hit','slam','block','special','notReady','throwBreak','holdBreak','kickout','guardBreak','count','roundEnd','matchEnd','jump','land','whiff','throw','pin','weapon','weaponBreak','taunt','tauntStart','climb','dive']);
