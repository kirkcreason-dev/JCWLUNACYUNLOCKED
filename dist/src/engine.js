import {fighterProfile,finisherMove} from './fighter-profile.js?v=0.19.1';
...
export const MOVES = Object.freeze({
  ...
  bottle: {startup:.16, active:.13, recovery:.30, reach:128, damage:11, stun:.36, knock:33, meter:10},
...
const unavailable=f=>['down','rise','grabbed','lifted','thrown','pinned','pin','climb','perch','dive'].includes(f.state);
export class Match {
  ...
  step(inputs=[emptyInput(),emptyInput()],dt=STEP){
    ...
    else if(this.options.mode!=='local'&&this.options.mode!=='online') inputs=[inputs[0],this.hitStop>0?this.aiInput:this.cpu(dt)];
  ...
  updateFighter(f,enemy,input,dt,index){
    ...
    if(canAct&&command==='weapon'&&f.z===0){
      f.buffer=null;const choices=f.definition.weapons?['none',...f.definition.weapons]:['chair'];
      f.weapon=choices[(choices.indexOf(f.weapon)+1)%choices.length];f.weaponUses=0;f.state='equip';f.t=0;
  ...
  resolveAttack({index,move,facing,blocked,combo,chain=0,counter=false,running=false}){
    ...
    if(f.weapon==='guitar'&&!['light','counter','aerial'].includes(move)){f.weaponUses++;if(f.weaponUses>=2){f.weapon='none';this.emit('weaponBreak',{index});}}
    if(move==='counter'){this.knockDown(e,'back',.28,1.35);}
    else if(chain===2||move==='special'||e.hp<=0||(['heavy','guitar','trashcan'].includes(move)&&e.hp<42)){this.knockDown(e,'back',.38,e.hp<25?3.7:2.5);}
  }
  secondWind(index){
  ...
  updateRopes(f,enemy,input,dt,index){
    ...
    if(f.state==='perch'){
      ...
      if(input.block||f.t>3){f.state='jump';f.t=0;f.vz=0;f.x=clamp(f.x+f.facing*45,LEFT,RIGHT);f.buffer=null;return true;}
  ...
  cpu(dt){
    ...
    if(enemy.move&&distance<165&&this.random()<(diff==='easy'?.28:diff==='hard'?.85:.6))v.block=true;
      ...
      else if(distance<MOVES[['bat','guitar','trashcan','bottle'].includes(me.weapon)?me.weapon:'heavy'].reach-8&&r<Math.min(.97,profile.heavy+.20))v.heavy=true;
