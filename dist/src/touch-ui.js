import {THROW_BREAK_WINDOW,LEFT,RIGHT,canPin as pinInRange,nearRopes,escapeTarget} from './engine.js?v=0.19.0';
...
export function touchContext(match,localIndex=0){
  ...
  return {escape,escapeLabel:pin?(match.pin.kind==='submission'?'ESCAPE':'KICK OUT'):'GET UP',escapeProgress:pin?Math.min(1,match.pin.escape/need):0,
    ...
    weapon:(me.weapon||'chair').toUpperCase(),heavy:perched?'DIVE':me.z>0?'AIR STRIKE':me.state==='block'?'COUNTER':me.weapon==='none'?(me.definition?.animations?.kick?'KICK':'STRIKE'):(me.weapon||'chair').toUpperCase(),ready:me.meter>=100,meter:Math.floor(me.meter),hint};
}
